...
function addReview(bookId, review) {
  ...
  utilService.saveToStorage(BOOKS_KEY, booksDB);
  return Promise.resolve();
}
...
function addGoogleBook(googleBook) {
  if (googleBook.saleInfo.saleability === 'NOT_FOR_SALE') return Promise.reject('Book is not for sale.');
  if (!googleBook.volumeInfo.publishedDate) return Promise.reject('Publish date error.');
  if (!googleBook.saleInfo.listPrice) return Promise.reject('Book price error.');
  var book = {
    id: googleBook.id,
    title: googleBook.volumeInfo.title,
    subtitle: googleBook.volumeInfo.subtitle,
    authors: googleBook.volumeInfo.authors,
    publishedDate: +googleBook.volumeInfo.publishedDate.split('-', 1),
